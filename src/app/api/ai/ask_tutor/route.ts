...
type TutorQuestionComplexity = "simple" | "complex";
type TutorResponseProfile = "adaptive_simple" | "adaptive_complex" | "forced_short" | "forced_detailed";
...
const TUTOR_SHORT_MAX_TOKENS = 520;
const TUTOR_DETAILED_MAX_TOKENS = 1200;
...
function normalizeAnswerMode(value: unknown): TutorAnswerMode {
    const normalized = safeString(value);
    if (normalized === "simple" || normalized === "detailed") return normalized;
    return "adaptive";
}

function detectTutorQuestionComplexity(question: string): TutorQuestionComplexity {
    const normalized = question.trim();
    const lower = normalized.toLowerCase();
    const punctuationCount = (normalized.match(/[，,。；;、]/g) ?? []).length;

    const hasComplexSignal = (
        /(为什么|原因|区别|对比|详细|深入|全面|系统|完整|逐句|逐词|展开|步骤|推导|多角度|结构分析|语法结构|详细讲讲)/u.test(normalized)
    ...
    const hasSimpleSignal = (
        /(什么意思|啥意思|怎么翻译|怎么说|怎么理解|这个词|这个短语|词义|短语)/u.test(normalized)
        || /(what does .* mean|meaning of|translate|translation|what is|what does)/i.test(lower)
    );
...
function resolveTutorResponseProfile(
    answerMode: TutorAnswerMode,
    complexity: TutorQuestionComplexity,
): TutorResponseProfile {
    if (answerMode === "simple") return "forced_short";
    if (answerMode === "detailed") return "forced_detailed";
    return complexity === "simple" ? "adaptive_simple" : "adaptive_complex";
}

function getTutorMaxTokens(responseProfile: TutorResponseProfile) {
    return responseProfile === "adaptive_simple" || responseProfile === "forced_short"
        ? TUTOR_SHORT_MAX_TOKENS
        : TUTOR_DETAILED_MAX_TOKENS;
}
...
${repairMode ? '17) REPAIR MODE: 这次回复必须补齐之前缺失字段，并强制贴合当前题目上下文。' : ""}
    ...
    if (intent === "rebuild") {
        ...
        const lengthControlInstructions = responseProfile === "adaptive_simple" || responseProfile === "forced_short"
            ? `
## 回答长度控制
- Answer Length Mode: "${answerMode}"
- Detected Complexity: "${questionComplexity}"
- Response Profile: "${responseProfile}"
- 用 2-4 行完成核心回答。
- 先直接回答，再补 1-2 个关键解释点。
- 除非用户明确要求展开、对比或例句，否则不要主动铺开。`
            : `
## 回答长度控制
- Answer Length Mode: "${answerMode}"
- Detected Complexity: "${questionComplexity}"
- Response Profile: "${responseProfile}"
- 可以适度展开，但控制在 2-3 段或最多 3 条短列表。
- 第一段先直答当前问题，再补“为什么这样写”。
- 只有真正帮助理解时才补对比、结构拆解或例句。`;
...
async function runNonStreamModel(
    prompt: string,
...
export async function POST(req: NextRequest) {
    try {
        ...
        const questionComplexity = detectTutorQuestionComplexity(normalizedQuestion);
        const responseProfile = resolveTutorResponseProfile(normalizedAnswerMode, questionComplexity);
        const maxTokens = getTutorMaxTokens(responseProfile);
        ...
        if (!isStreaming) {
            let content = await runNonStreamModel(makePrompt(false), normalizedThinkingMode, maxTokens);
            let result = buildValidatedPayload(content);

            if (result.shouldRetry) {
                content = await runNonStreamModel(makePrompt(true), normalizedThinkingMode, maxTokens);
