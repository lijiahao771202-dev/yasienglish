...
describe("ai ask route", () => {
    ...
    it("uses sentence teaching instructions when the user asks to break down a full sentence", async () => {
        ...
        expect(completionParams.messages[0].content).toContain("Don't drift into general study advice, exam tips, or life lessons unless the user asks for them.");
        expect(completionParams.messages[0].content).toContain("Use these sections in this order");
        expect(completionParams.messages[0].content).toContain("go through the sentence left to right, chunk by chunk, end-to-end");
        expect(completionParams.messages[0].content).toContain("Lead with prose; treat tables as an OPTIONAL supplementary aid");
        expect(completionParams.messages[0].content).toContain("Split the sentence into 2-4 natural sense groups");
        expect(completionParams.messages[0].content).toContain("Most of the teaching weight lives here, not in tables");
        expect(completionParams.messages[0].content).toContain("pick whichever columns best fit what you are showing");
        expect(completionParams.messages[0].content).toContain("Use 2-4 columns");
        expect(completionParams.messages[0].content).toContain("Do NOT add a table to every group");
        expect(completionParams.messages[0].content).toContain("highest-value words or collocations from this sentence");
        expect(completionParams.messages[0].content).toContain("Don't underdeliver");
        ...
        expect(completionParams.messages[0].content).toContain("English-with-Chinese gloss policy");
        expect(completionParams.messages[0].content).toContain("immediately follow it with a concise Chinese gloss in full-width parentheses");
        expect(completionParams.messages[0].content).toContain("Do not leave important inline English fragments bare");
        expect(completionParams.messages[0].content).toContain("`This narrative`（这种叙述）");
        expect(completionParams.messages[0].content).toContain("`that`（引导宾语从句）");
        expect(completionParams.messages[0].content).toContain("Never leave a bare English chunk inside any sub-heading");
        ...
        expect(completionParams.messages[0].content).toContain("most important takeaway");
        ...
        expect(completionParams.messages[0].content).toContain("In 结构拆解, you may organise the chunk-by-chunk breakdown as a Markdown table");
