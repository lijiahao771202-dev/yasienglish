...
describe("ai ask route", () => {
    ...
    it("uses sentence teaching instructions when the user asks to break down a full sentence", async () => {
        ...
        expect(completionParams.messages[0].content).toContain("each block title starts with the exact English chunk followed immediately by its Chinese meaning");
        expect(completionParams.messages[0].content).toContain("Each mini block should follow this exact shape");
        expect(completionParams.messages[0].content).toContain("**`English chunk`（中文意思）**");
        expect(completionParams.messages[0].content).toContain("语法功能");
        expect(completionParams.messages[0].content).toContain("说明");
        ...
        expect(completionParams.messages[0].content).toContain("immediately add a concise Chinese gloss in full-width parentheses");
        ...
        expect(completionParams.messages[0].content).toContain("In 结构拆解 numbered mini-block titles, put the Chinese gloss directly after the English chunk");
