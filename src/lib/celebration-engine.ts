...
let activeIntervalObjs: { cleared: boolean }[] = [];
...
export const clearAllCelebrations = () => {
    activeTimeouts.forEach(window.clearTimeout);
    activeTimeouts.clear();
    activeIntervals.forEach(window.clearInterval);
    activeIntervals.clear();
    activeIntervalObjs.forEach(o => { o.cleared = true; });
    activeIntervalObjs = [];
    activeOscillators.forEach(osc => {
        try { osc.onended = null; if ('stop' in osc) osc.stop(); osc.disconnect(); } catch (e) { }
    });
...
const SHAPES = [['circle'], ['square'], ['star'], ['circle', 'square'], ['square', 'star']];

// DETERMINISTIC RANDOM
const seedRand = (str: string) => {
    let h = 0;
    for(let i=0; i<str.length; i++) h = Math.imul(31, h) + str.charCodeAt(i) | 0;
    return () => { h = Math.imul(741103597, h); return (h >>> 0) / 4294967296; };
};

const getShapes = (emojis: string[] | null, shapes: string[] | null): any => {
    if (emojis && (confetti as any).shapeFromText) {
        return emojis.map(e => (confetti as any).shapeFromText({ text: e, scalar: 2 }));
    }
...
const buildCommonVisual = (type: string, count: number, v: number, grav: number, decay: number, scalar: number, colors: string[], emojis: string[]|null, shapes: string[]|null, originX: number) => {
    return (origin: {x:number, y:number}, rm: boolean) => {
        ...
        const ox = originX !== 0.5 ? originX : origin.x; // override if needed
        const scalarFinal = rm ? scalar * 0.8 : scalar;

        if (type === 'burst') {
            confetti({ particleCount: c, spread: 80, origin: {x: ox, y: origin.y}, startVelocity: v, colors, shapes: sh, gravity: grav, decay, scalar: scalarFinal });
            safeSetTimeout(() => confetti({ particleCount: c*0.5, spread: 100, origin: {x: ox, y: origin.y}, startVelocity: v*0.8, colors, shapes: sh, gravity: grav*1.2, decay, scalar: scalarFinal }), 50);
        } else if (type === 'bubbles') {
            confetti({ particleCount: c, spread: 180, origin, startVelocity: v, colors, shapes: sh, gravity: grav, decay, scalar: scalarFinal*1.5 });
        } else if (type === 'heavy-drop') {
            confetti({ particleCount: c, spread: 50, origin, startVelocity: v, colors, shapes: sh, gravity: grav, decay, scalar: scalarFinal*1.2 });
        } else if (type === 'sideways-wind') {
            confetti({ particleCount: c, angle: 0, spread: 30, origin: {x: 0, y: origin.y}, startVelocity: v, colors, shapes: sh, gravity: grav, decay, scalar: scalarFinal });
        } else if (type === 'laser-spear') {
            confetti({ particleCount: c, spread: 10, origin, startVelocity: v, colors, shapes: sh, gravity: grav, decay, scalar: scalarFinal*0.3 });
        } else if (type === 'jelly-bounce') {
            confetti({ particleCount: c, spread: 45, angle: 90, origin, startVelocity: v, colors, shapes: sh, gravity: 0.5, decay: 0.9, scalar: scalarFinal*1.5 });
            safeSetTimeout(() => confetti({ particleCount: c, spread: 90, angle: 270, origin, startVelocity: v*0.5, colors, shapes: sh, gravity: 2, decay: 0.95, scalar: scalarFinal }), 80);
        } else if (type === 'micro-dust') {
            confetti({ particleCount: c, spread: 360, origin, startVelocity: v, colors, shapes: sh, gravity: grav, decay, scalar: scalarFinal*0.4 });
        } else if (type === 'implode') {
            confetti({ particleCount: c, spread: 360, origin, startVelocity: -v, colors, shapes: sh, gravity: grav, decay: 1.05, scalar: scalarFinal });
        }
    }
};
...
interface CelebrationTheme {
    ...
    visualFn: (origin: {x:number, y:number}, rm: boolean) => void;
    audioFn: (ctx: AudioContext) => void;
}

const buildThemes = () => {
    const list: CelebrationTheme[] = [];
    let id = 1;

    // 1. Common (1-50)
    let rnd = seedRand('common_seed_4');
    const commonVisuals = [
    ...
    const palKeys = Object.keys(PALETTES) as (keyof typeof PALETTES)[];

    for(let i=0; i<50; i++) {
        const v = commonVisuals[Math.floor(rnd() * commonVisuals.length)];
        const a = commonAudio[Math.floor(rnd() * commonAudio.length)];
        const palName = palKeys[Math.floor(rnd() * palKeys.length)];
        const pal = PALETTES[palName];
        
        let usesEmoji = rnd() > 0.6;
        let emoji = usesEmoji ? EMOJIS[Math.floor(rnd() * EMOJIS.length)] : null;
        let shapes = usesEmoji ? null : SHAPES[Math.floor(rnd() * SHAPES.length)];

        list.push({
            id: id++, rarity: 'Common', name: `${palName} ${v.title} & ${a.title}`,
            visualFn: buildCommonVisual(v.type, v.count, v.v, v.grav, v.decay, v.scalar || 1, pal, emoji, shapes, v.x || 0.5),
            audioFn: buildCommonAudio(a.type, a)
        });
    }
    ...
    for(let i=0; i<30; i++) {
        const v = rareVisuals[Math.floor(rnd() * rareVisuals.length)];
        const a = rareAudio[Math.floor(rnd() * rareAudio.length)];
        const palName = palKeys[Math.floor(rnd() * palKeys.length)];
        
        let usesEmoji = rnd() > 0.4;
        let emoji = usesEmoji ? EMOJIS[Math.floor(rnd() * EMOJIS.length)] : null;
        let shapes = usesEmoji ? null : ['star', 'circle'];

        list.push({
            id: id++, rarity: 'Rare', name: `Rare ${palName} ${v.title}`,
            visualFn: buildRareVisual(v.type, v.count, PALETTES[palName], emoji, shapes),
            audioFn: buildRareAudio(a.type, a)
        });
    }
    ...
    for(let i=0; i<15; i++) {
        list.push({
            ...
            visualFn: buildEpicVisual(i),
            audioFn: buildEpicAudio(i)
        });
    }

    // 4. LEGENDARY (96-100)
    list.push({
        id: id++, rarity: 'Legendary', name: 'The Void',
        visualFn: (origin: any, rm: boolean) => {
    ...
    list.push({
        id: id++, rarity: 'Legendary', name: 'Golden Nuke',
        visualFn: (o: any, rm: boolean) => {
    ...
    list.push({
        id: id++, rarity: 'Legendary', name: 'Glitch Reality',
        visualFn: (o: any, rm: boolean) => {
    ...
    list.push({
        id: id++, rarity: 'Legendary', name: 'Time Stop',
        visualFn: (o: any, rm: boolean) => {
    ...
    list.push({
        id: id++, rarity: 'Legendary', name: 'Hypernova',
        visualFn: (o: any, rm: boolean) => {
...
export const getRandomCelebrationTheme = (): CelebrationTheme => {
