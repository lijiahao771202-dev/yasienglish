alter table public.profiles
  add column if not exists dictation_elo integer not null default 400,
  add column if not exists dictation_streak integer not null default 0,
  add column if not exists dictation_max_elo integer not null default 400;

update public.profiles
set
  dictation_elo = coalesce(dictation_elo, listening_elo, 400),
  dictation_streak = coalesce(dictation_streak, 0),
  dictation_max_elo = greatest(
    coalesce(dictation_max_elo, 0),
    coalesce(dictation_elo, listening_elo, 400)
  );
...
alter table public.elo_history
  ...
  check (mode in ('translation', 'listening', 'rebuild', 'dictation'));
...
begin
  ...
  if p_mode not in ('translation', 'listening', 'rebuild', 'dictation') then
  ...
  values (
    ...
    400,
    case when p_mode = 'dictation' then p_elo_after else 400 end,
    ...
    case when p_mode = 'dictation' then p_streak_count else 0 end,
    ...
    case when p_mode = 'dictation' then p_max_elo else 400 end,
  ...
  set
    ...
    dictation_elo = case when p_mode = 'dictation' then p_elo_after else public.profiles.dictation_elo end,
    ...
    dictation_streak = case when p_mode = 'dictation' then p_streak_count else public.profiles.dictation_streak end,
    ...
    dictation_max_elo = case when p_mode = 'dictation' then greatest(public.profiles.dictation_max_elo, p_max_elo) else public.profiles.dictation_max_elo end,
