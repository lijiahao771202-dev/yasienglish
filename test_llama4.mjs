...
async function run() {
    console.log("🔍 Fetching all Llama 4 models...");
    const modelsResponse = await client.models.list();
    const llama4Models = modelsResponse.data
        .filter(m => m.id.toLowerCase().includes("llama-4"))
        .map(m => m.id);
    
    console.log("Found Llama 4 models:", llama4Models);
...
`;
    ...
    console.log("\n🔥 Benchmarking Llama 4 Models on NVIDIA NIM...\n");
    console.log("=".repeat(100));

    for (const modelId of llama4Models) {
